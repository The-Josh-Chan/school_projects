...
int delta  = 45;   // 25 as default for speed for speed test
int HYS    = 100;   // 50 as default for hysterisis
...
void loop() {
  ...
  do {
    ...
    if (error > 10 ) { //too far from the wall
      turnOnLED(YLW);
      runMotors(deltaV, delta);
    }

    else if (error < -10){ //too close to the wall
      turnOnLED(RED);
